...
struct OutlierParams {
    ...
    double radius_search;  // 搜索半径
    int min_neighbors;     // 最小邻居数量
};

// *** 新增：自过滤参数结构 ***
struct SelfFilterParams {
    bool enable_self_filter;
    double filter_radius;  // 自过滤半径
};
...
public:
    MapAccumulatorWithDisROI(ros::NodeHandle& nh) 
        ...
        private_nh_.param("map_roi_min_x", map_roi_params_.min_x, -5.0);
        private_nh_.param("map_roi_max_x", map_roi_params_.max_x, 10.00);
        private_nh_.param("map_roi_min_y", map_roi_params_.min_y, -3.0);
        private_nh_.param("map_roi_max_y", map_roi_params_.max_y, 7.0);
        private_nh_.param("map_roi_min_z", map_roi_params_.min_z, 0.4);
        ...
        // *** 新增：孤立点过滤参数 ***
        private_nh_.param("enable_outlier_filter", outlier_params_.enable_outlier_filter, true);
        private_nh_.param("outlier_radius_search", outlier_params_.radius_search, 0.1);
        private_nh_.param("outlier_min_neighbors", outlier_params_.min_neighbors, 1);

        // *** 新增：自过滤参数 ***
        private_nh_.param("enable_self_filter", self_filter_params_.enable_self_filter, true);
        private_nh_.param("self_filter_radius", self_filter_params_.filter_radius, 1.0);
        ...
        outlier_filtered_points_ = 0;  // *** 新增：孤立点过滤统计初始化 ***
        self_filtered_points_ = 0;     // *** 新增：自过滤统计初始化 ***
        ...
        // *** 新增：孤立点过滤信息输出 ***
        if (outlier_params_.enable_outlier_filter) {
            ROS_INFO("Outlier Filter enabled: radius=%.2fm, min_neighbors=%d", 
                     outlier_params_.radius_search, outlier_params_.min_neighbors);
            ROS_INFO("Points with less than %d neighbors within %.2fm will be removed as noise.", 
                     outlier_params_.min_neighbors, outlier_params_.radius_search);
        } else {
             ROS_INFO("Outlier Filter is DISABLED.");
        }

        // *** 新增：自过滤信息输出 ***
        if (self_filter_params_.enable_self_filter) {
            ROS_INFO("Self Filter enabled: radius=%.2fm", self_filter_params_.filter_radius);
            ROS_INFO("Points within %.2fm of UAV position will be filtered out.", 
                     self_filter_params_.filter_radius);
        } else {
             ROS_INFO("Self Filter is DISABLED.");
        }
    ...
    ~MapAccumulatorWithDisROI() {
        ...
        ROS_INFO("Outlier filtered points: %zu (%.2f%%)", outlier_filtered_points_,
                 total_points_received_ > 0 ? 100.0 * outlier_filtered_points_ / total_points_received_ : 0.0);
        ROS_INFO("Self filtered points: %zu (%.2f%%)", self_filtered_points_,
                 total_points_received_ > 0 ? 100.0 * self_filtered_points_ / total_points_received_ : 0.0);
...
private:
    ...
    OutlierParams outlier_params_;   // *** 新增：孤立点过滤参数 ***
    SelfFilterParams self_filter_params_;  // *** 新增：自过滤参数 ***
    ...
    size_t outlier_filtered_points_;  // *** 新增：孤立点过滤统计 ***
    size_t self_filtered_points_;     // *** 新增：自过滤统计 ***
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr filterOutliers(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud);  // *** 新增：孤立点过滤 ***
    pcl::PointCloud<pcl::PointXYZ>::Ptr filterSelf(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud);  // *** 新增：自过滤 ***
...
void MapAccumulatorWithDisROI::odomCallback(const nav_msgs::OdometryConstPtr& odom_msg) {
    ...
    // *** 新增：更新当前位置用于自过滤 ***
    {
        ...
        current_position_ = odom_msg->pose.pose.position;
        has_odom_data_ = true;
    }
}
...
// *** 新增：孤立点过滤函数（移除0.1m内没有足够邻居的点）***
pcl::PointCloud<pcl::PointXYZ>::Ptr MapAccumulatorWithDisROI::filterOutliers(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud) {
    if (cloud->empty() || !outlier_params_.enable_outlier_filter) {
        return cloud; // 如果未启用孤立点过滤，直接返回原云
    }

    // 使用 PCL 的 RadiusOutlierRemoval 过滤器
    pcl::RadiusOutlierRemoval<pcl::PointXYZ> ror;
    ror.setInputCloud(cloud);
    ror.setRadiusSearch(outlier_params_.radius_search);  // 搜索半径 (默认0.1m)
    ror.setMinNeighborsInRadius(outlier_params_.min_neighbors);  // 最小邻居数量 (默认1)

    pcl::PointCloud<pcl::PointXYZ>::Ptr filtered_cloud(new pcl::PointCloud<pcl::PointXYZ>);
    ror.filter(*filtered_cloud);

    // *** 统计过滤的点数 ***
    size_t filtered_count = cloud->size() - filtered_cloud->size();
    outlier_filtered_points_ += filtered_count;

    ROS_DEBUG("Outlier filtering: %zu -> %zu points (%zu outliers removed)", 
             cloud->size(), filtered_cloud->size(), filtered_count);

    return filtered_cloud;
}

// *** 新增：自过滤函数（移除距离UAV位置过近的点）***
pcl::PointCloud<pcl::PointXYZ>::Ptr MapAccumulatorWithDisROI::filterSelf(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud) {
    if (cloud->empty() || !self_filter_params_.enable_self_filter) {
        return cloud; // 如果未启用自过滤，直接返回原云
    }

    // 检查是否有里程计数据
    geometry_msgs::Point uav_position;
    bool has_position = false;
    {
        std::lock_guard<std::mutex> lock(odom_mutex_);
        if (has_odom_data_) {
            uav_position = current_position_;
            has_position = true;
        }
    }

    if (!has_position) {
        ROS_DEBUG("No odometry data available for self filtering, skipping");
        return cloud;
    }
    ...
    // 手动遍历每个点，检查是否距离UAV过近
    for (const auto& point : cloud->points) {
        // 计算点到UAV位置的2D距离（仅考虑x,y，忽略z高度差）
        double dx = point.x - uav_position.x;
        double dy = point.y - uav_position.y;
        double distance_2d = sqrt(dx * dx + dy * dy);
        
        if (distance_2d > self_filter_params_.filter_radius) {
            // 如果点距离UAV超过过滤半径，则保留
            filtered_cloud->points.push_back(point);
        } else {
            // 如果点距离UAV太近，则过滤掉（计数）
    ...
    // *** 统计过滤的点数 ***
    self_filtered_points_ += filtered_count;

    ROS_DEBUG("Self filtering: %zu -> %zu points (%zu self-filtered), UAV pos: (%.2f, %.2f, %.2f)", 
             cloud->size(), filtered_cloud->size(), filtered_count,
...
void MapAccumulatorWithDisROI::addCloudToMap(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud) {
    ...
    // *** 处理流程：1. 自过滤（移除） 2. DisROI过滤（移除） 3. ROI过滤（保留） 4. 孤立点过滤（移除） 5. 降采样 6. 添加到缓冲区 ***
    
    // 步骤1：应用自过滤（移除距离UAV过近的点）
    pcl::PointCloud<pcl::PointXYZ>::Ptr self_filtered_cloud = filterSelf(cloud);
    if (self_filtered_cloud->empty()) {
        ROS_DEBUG("All points filtered out by self filtering, skipping addition to map.");
        return;
    }

    // 步骤2：应用DisROI过滤（移除DisROI内的点）
    pcl::PointCloud<pcl::PointXYZ>::Ptr disroi_filtered_cloud = filterDisROI(self_filtered_cloud);
    if (disroi_filtered_cloud->empty()) {
        ROS_DEBUG("All points filtered out by DisROI, skipping addition to map.");
        return;
    }

    // 步骤3：应用Map ROI过滤（保留ROI内的点）
    ...
    // 步骤4：应用孤立点过滤（移除噪声点）
    pcl::PointCloud<pcl::PointXYZ>::Ptr outlier_filtered_cloud = filterOutliers(roi_filtered_cloud);
    if (outlier_filtered_cloud->empty()) {
        ROS_DEBUG("All points filtered out by outlier removal, skipping addition to map.");
        return;
    }

    // 步骤5：降采样过滤后的点云
    pcl::PointCloud<pcl::PointXYZ>::Ptr downsampled_cloud(new pcl::PointCloud<pcl::PointXYZ>(*outlier_filtered_cloud));
    ...
    ROS_DEBUG("Added %zu points (after self/DisROI/ROI/outlier/downsample), total: %zu in buffer with %zu frames", 
...
void MapAccumulatorWithDisROI::publishMapInfo() {
    ...
    if (total_points_received_ > 0) {
        ...
        ss << "Self filtered: " << self_filtered_points_
           << " (" << std::fixed << std::setprecision(1) 
           << 100.0 * self_filtered_points_ / total_points_received_ << "%)\n";
        ...
        ss << "Outlier filtered: " << outlier_filtered_points_
           << " (" << std::fixed << std::setprecision(1) 
           << 100.0 * outlier_filtered_points_ / total_points_received_ << "%)\n";
    ...
    if (outlier_params_.enable_outlier_filter) {
        ss << "Outlier Filter: radius=" << outlier_params_.radius_search 
           << "m, min_neighbors=" << outlier_params_.min_neighbors << "\n";
    }
    if (self_filter_params_.enable_self_filter) {
        ss << "Self Filter: radius=" << self_filter_params_.filter_radius << "m\n";
    }
    ...
    if (!accumulated_cloud_->empty()) {
        ...
        marker.text = "Points: " + std::to_string(accumulated_cloud_->size()) + 
                     ...
                     "\nSelf: " + std::to_string(self_filtered_points_) +
                     "\nDisROI: " + std::to_string(disroi_filtered_points_) +
                     "\nOutliers: " + std::to_string(outlier_filtered_points_);
    ...
    try {
        ...
        if (result == 0) {
            ...
            if (stats_file.is_open()) {
                ...
                stats_file << "Self filtered points: " << self_filtered_points_ 
                          << " (" << (total_points_received_ > 0 ? 100.0 * self_filtered_points_ / total_points_received_ : 0.0) << "%)\n";
                stats_file << "DisROI filtered points: " << disroi_filtered_points_ 
                          << " (" << (total_points_received_ > 0 ? 100.0 * disroi_filtered_points_ / total_points_received_ : 0.0) << "%)\n";
                stats_file << "ROI filtered points: " << roi_filtered_points_
                          << " (" << (total_points_received_ > 0 ? 100.0 * roi_filtered_points_ / total_points_received_ : 0.0) << "%)\n";
                stats_file << "Outlier filtered points: " << outlier_filtered_points_
                          << " (" << (total_points_received_ > 0 ? 100.0 * outlier_filtered_points_ / total_points_received_ : 0.0) << "%)\n";
                ...
                if (self_filter_params_.enable_self_filter) {
                    stats_file << "Self Filter: radius=" << self_filter_params_.filter_radius << "m\n";
                }
                if (disroi_params_.enable_disroi_filter) {
                ...
                if (outlier_params_.enable_outlier_filter) {
                    stats_file << "Outlier Filter: radius=" << outlier_params_.radius_search 
                              << "m, min_neighbors=" << outlier_params_.min_neighbors << "\n";
                }
    ...
    while (ros::ok()) {
        // 检查是否需要保存
        {
            ...
            if (clear_requested_) {
                ...
                disroi_filtered_points_ = 0;
                roi_filtered_points_ = 0;
                outlier_filtered_points_ = 0;  // *** 新增：重置孤立点过滤统计 ***
                self_filtered_points_ = 0;     // *** 新增：重置自过滤统计 ***
                accumulated_points_ = 0;
